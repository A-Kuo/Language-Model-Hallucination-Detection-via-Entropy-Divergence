...
Usage:
    analyzer = AttentionAnalyzer(model_name="EleutherAI/pythia-160m")
...
class AttentionAnalyzer:
    ...
    ----------
    model_name : str
        HuggingFace model ID (default "EleutherAI/pythia-160m", 160M params).
    device : str | None
    ...
    def __init__(
        ...
        self.num_layers: int = getattr(config, "num_hidden_layers", getattr(config, "n_layer", None))
        self.num_heads: int = getattr(config, "num_attention_heads", getattr(config, "n_head", None))
...
if __name__ == "__main__":
    ...
    print("To run with a model, use: AttentionAnalyzer(model_name='EleutherAI/pythia-160m')")
    print(f"{'=' * 60}")
