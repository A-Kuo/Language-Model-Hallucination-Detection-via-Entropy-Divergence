...
to semantic depth, achieving an AUROC of \RESULT{0.781} on the HaluEval 
question-answering split evaluated on Pythia-160m.
A logistic regression baseline on the same full AED features achieves \RESULT{0.907},
indicating that the extracted features are highly discriminative.
An ablation study confirms that attention entropy (AUROC $0.823$) is the 
dominant signal, while cross-layer KL divergence alone (AUROC $0.540$) provides 
limited discriminative power; their combination (AUROC $0.830$) yields the 
strongest overall feature set.
%
...
following \citet{chuang2024lookback} (Lookback Lens), who showed that attention
patterns at the last generated token correlate with factual accuracy, and
extending this line of work with spectral \citep{binkowski2025spectral} and
frequency-domain \citep{qi2026frequency} attention features.
...
\citet{chuang2024lookback} (Lookback Lens) compute the ratio of attention 
...
Algorithm~\ref{alg:aed} gives a complete specification of AED feature extraction.

\begin{algorithm}[h]
\caption{AED Feature Extraction}
\label{alg:aed}
\begin{algorithmic}[1]
\Require Transformer $\mathcal{M}$ with $L$ layers and $H$ heads,
         input sequence $\mathbf{x} = (x_1, \ldots, x_T)$
\Ensure Feature vector $\mathbf{f} \in \mathbb{R}^{2L}$
\State Run $\mathcal{M}(\mathbf{x})$ with \texttt{output\_attentions=True}
\State Collect $\{\mathbf{A}^{l,h}\}_{l=1}^{L}$, \quad shape each: $T \times T$
\For{$l = 1$ \textbf{to} $L$}
  \For{$h = 1$ \textbf{to} $H$}
    \State $\mathbf{a}^{l,h} \leftarrow \mathbf{A}^{l,h}_{T,:}$ \Comment{Last-token attention row}
    \State $H_{l,h} \leftarrow -\sum_j a^{l,h}_j \log_2 a^{l,h}_j$
  \EndFor
  \State $\bar{H}^l \leftarrow \frac{1}{H}\sum_{h=1}^{H} H_{l,h}$ \Comment{Mean entropy at layer $l$}
  \State $\bar{\mathbf{a}}^l \leftarrow \frac{1}{H}\sum_{h=1}^{H} \mathbf{a}^{l,h}$ \Comment{Mean attn distribution}
  \If{$l > 1$}
    \State $D^l \leftarrow D_{\text{KL}}(\bar{\mathbf{a}}^{l-1} \| \bar{\mathbf{a}}^l)$
  \Else
    \State $D^l \leftarrow 0$
  \EndIf
\EndFor
\State \Return $\mathbf{f} = (\bar{H}^1, \ldots, \bar{H}^L, D^1, \ldots, D^L)$
\end{algorithmic}
\end{algorithm}
...
\paragraph{Baselines:}
\begin{itemize}
  \item \textbf{Lookback Ratio} \citep{chuang2024lookback}: context attention 
...
\midrule
AED-LogReg (full features)              & \RESULT{0.907} & \RESULT{0.815} & \RESULT{0.30} & 25 \\
AED-BiLSTM (layer sequence)             & \RESULT{0.781} & \RESULT{0.826} & \RESULT{0.38} & $\sim$6K \\
\bottomrule
\end{tabular}
\end{table}

All values in Table~\ref{tab:results} are from actual experimental runs
on a T4 GPU via the reproducible Colab notebook (\texttt{colab/gpu\_benchmark.ipynb}).
The LogReg baseline achieves higher AUROC, indicating that the AED features 
are strongly discriminative and that the simplified BiLSTM does not yet 
fully exploit the sequential structure of the per-layer signal.

\subsection{Ablation: Which Signal Contributes More?}

We ablate the two signal components---entropy and KL divergence---to 
determine their relative contribution.  Table~\ref{tab:ablation} shows 
results using the BiLSTM classifier on 200 HaluEval QA samples.
...
\begin{tabular}{lcc}
\toprule
\textbf{Features} & \textbf{AUROC} & \textbf{FPR@90\%TPR} \\
\midrule
Entropy only (no KL)    & \RESULT{0.823} & \RESULT{0.30} \\
KL divergence only      & \RESULT{0.540} & \RESULT{0.60} \\
Both (full AED)         & \RESULT{0.830} & \RESULT{0.30} \\
\bottomrule
\end{tabular}
\end{table}

Contrary to our initial hypothesis, attention entropy is the dominant signal 
(AUROC $0.823$), while KL divergence alone performs near random chance ($0.540$).
The combined feature set provides a marginal improvement ($0.830$), suggesting 
that KL divergence adds complementary information at the margin but does not 
independently capture the hallucination pattern.
This finding aligns with the information-theoretic intuition: entropy directly 
measures uncertainty in the model's attention allocation, while KL divergence 
captures \emph{change} in attention across layers---a second-order signal that 
is noisier on the small Pythia-160m architecture.

\subsection{Layer Depth Analysis}

Figure~\ref{fig:layer_entropy} (generated by \texttt{colab/gpu\_benchmark.ipynb}) 
plots mean attention entropy by layer for hallucinated vs.\ non-hallucinated 
samples.  The divergence between the two curves increases in later layers, 
consistent with the hypothesis that deeper layers encode factual grounding 
while early layers capture syntactic structure.

\subsection{Latency Analysis}

AED feature extraction adds \RESULT{38.5}\,ms per sample on a T4 GPU 
(Pythia-160m, 128-token sequences).  The attention tensors are produced 
during the normal forward pass (\texttt{output\_attentions=True}); the 
additional cost is the entropy and KL computation over $L \times H$ 
attention matrices, which is negligible compared to the forward pass itself.
...
BiLSTM, achieving AUROC 0.781 on HaluEval with Pythia-160m (0.907 
with logistic regression on the same features).
